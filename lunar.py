...
class Lunar():
    ...
    def get_AngelDemon(self):
        ...
        angel = [
            ...
            ('吉期', '丑寅卯辰巳无为申酉戌亥子'[men], d, ['施恩', '举正直', '出行', '上官', '临政'], []),
            ('小时', '子丑寅卯辰巳无为申酉戌亥'[men], d, [], []),
            ('兵福', '子丑寅卯辰巳无为申酉戌亥'[men], d, ['安抚边境', '选将', '出师'], []),
            ('兵宝', '丑寅卯辰巳无为申酉戌亥子'[men], d, ['安抚边境', '选将', '出师'], []),
        ]
        demon = [
        ...
        if thingLevel != 3:
            ...
            if ['','甲戌','','丙申','','甲子','戊申','庚辰','辛卯','甲子','','甲子'][lmn-1] in d:
                self.badThing = ['诸事不忌']
