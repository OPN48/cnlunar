...
class Lunar():
    ...
    def get_AngelDemon(self):
        ...
        angel = [
            ...
            ('月德', '壬庚丙甲'[men % 4], d[0], ['赴任', '谒贵', '求贤', '修造', '动土', '嫁娶', '移徙', '纳财', '买畜', '市贾', '立契']),
        ...
        demon = [
            ...
            ('月厌', '子亥戌酉申未午巳辰卯寅丑'[men], d,
             ['祈福', '求嗣', '上表', '颁诏', '施恩', '诏命', '招贤', '布政', '庆赐', '赏贺', '宴会', '冠带', '行幸', '遣使', '安抚', '选将', '出师',
            ...
            ('往亡', '戌丑寅巳申亥卯午酉子辰未'[men], d,
             ['出行', '赴任', '嫁娶', '求谋', '求财', '上表', '颁诏', '诏命', '招贤', '布政', '行幸', '遣使', '安抚', '选将', '出师', '临政', '进人',
              '搬移', '求医', '捕捉', '畋猎', '取鱼']),
            ...
            ('天吏', '卯子酉午卯子酉午卯子酉午'[men], d,['祈福', '求嗣', '上表', '诏命', '招贤', '冠带', '行幸', '遣使', '安抚', '选将', '出师', '赴任', '临政','嫁娶','进人','搬移','安床','解除','求医','修造','动土','竖柱','修仓','开市','立券','交易','纳财','开仓','出货','修置','栽种','牧养','纳畜']),
            ...
            ('天贼', '卯寅丑子亥戌酉申未午巳辰'[men], d, ['安葬', '出行', '开池', '动土', '竖造', '入宅', '开仓', '行幸', '遣使', '修仓', '出货']),
            ('地火', '子亥戌酉申未午巳辰卯寅丑'[men], d, ['栽种', '修园']),
            ('独火', '未午巳辰卯寅丑子亥戌酉申'[men], d, ['作灶', '修造']),
            ('月破', '午未申酉戌亥子丑寅卯辰巳'[men], d,['修造', '大事勿用', '祈福', '求嗣', '上表', '颁诏', '诏命', '招贤', '布政', '庆赐', '赏贺', '宴会', '冠带', '行幸', '遣使', '安抚', '选将','出师', '赴任', '临政', '纳采', '嫁娶', '进人', '搬移', '安床', '整容', '剃头', '修甲', '裁制', '修造', '动土', '竖柱', '修仓', '鼓铸','经络', '酝酿', '立券', '交易', '纳财', '开仓', '出货', '修置', '开渠', '安碓', '塞穴', '修墙', '伐木', '栽种', '牧养', '纳畜', '破土','安葬', '启攒']),
            ...
            ('荒芜', d[1], '巳酉丑申子辰亥卯未寅午戌'[sn * 3:sn * 3 + 3], []),# ('四正废', d, '庚申辛酉壬子癸亥甲寅乙卯丙午丁巳'[sn * 4:sn * 4 + 4], ['修造', '交易', '安床']),
            ('四废', d, ('庚申辛酉', '壬子癸亥', '甲寅乙卯', '丁巳丙午')[sn], ['修造', '交易', '安床']),  # 庚申辛酉为春废，壬子癸亥夏时当。甲寅乙卯秋月值，丁巳丙午冬季防。
            ('蚩尤', '戌子寅辰午申'[men % 6], d, ['冠笄']),  # 正七逢寅二八辰，三九午上四十申。五十一月原在戌，六十二月子为真。
            ('大时', '酉午卯子酉午卯子酉午卯子'[men], d,['祈福', '求嗣', '上表', '颁诏', '施恩', '诏命', '招贤', '冠带', '行幸', '遣使', '安抚', '选将', '出师', '赴任','临政', '纳采', '嫁娶', '进人', '搬移', '安床', '解除', '求医', '营建', '修造', '竖柱', '修仓','开市', '立券', '交易', '纳财', '开仓', '出货', '修置', '栽种', '牧养', '纳畜']),
            # 《神枢经》曰:大时者,将军之象也。所直之日,忌出军攻战、筑室会亲。李鼎祚曰:大时者,正月起卯逆行四仲。
            ('大败', '酉午卯子酉午卯子酉午卯子'[men], d,['祈福', '求嗣', '上表', '颁诏', '施恩', '诏命', '招贤', '冠带', '行幸', '遣使', '安抚', '选将', '出师', '赴任','临政', '纳采', '嫁娶', '进人', '搬移', '安床', '解除', '求医', '营建', '修造', '竖柱', '修仓','开市', '立券', '交易', '纳财', '开仓', '出货', '产室', '栽种', '牧养', '纳畜', '出行', '营谋', '求婚']),
            ...
            ('土符', '申子丑巳酉寅午戌卯未亥辰'[men], d,['营建', '修造', '修仓', '修置', '开渠', '安碓', '补垣', '修墙', '平道', '破屋', '栽种', '破土','起造']),  # 《历例》曰:士符者,正月丑，二月已,三月酉,四月寅,五月午,六月戌,七月卯,八月未,九月亥,十月辰,十一月申,十二月子。
            ...
            ('月害', '未午巳辰卯寅丑子亥戌酉申'[men], d,['祈福', '求嗣', '上表', '庆赐', '赏贺', '宴会', '安抚', '选将', '出师', '纳采', '嫁娶', '进人', '求医', '修仓', '经络','酝酿', '开市', '立券', '交易', '纳财', '开仓', '出货', '修置', '牧养', '纳畜', '破土', '安葬', '启攒']),
            ('月刑', '卯戌巳子辰申午丑寅酉未亥'[men], d,['祈福', '求嗣', '上表', '颁诏', '施恩', '诏命', '招贤', '布政', '庆赐', '赏贺', '宴会', '冠带', '行幸', '遣使', '安抚', '训兵', '出师','赴任', '临政', '纳采', '嫁娶', '进人', '搬移', '安床', '解除', '整容', '剃头', '修甲', '求医','裁制', '营建', '修造', '竖柱', '修仓', '鼓铸', '经络', '酝酿', '开市', '立券', '交易', '纳财', '开仓','出货', '修置', '开渠', '安碓', '塞穴', '修墙', '破屋', '栽种', '牧养', '纳畜', '破土', '安葬', '启攒']),
